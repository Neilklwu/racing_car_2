...
MainWindow::MainWindow(QWidget *parent): QMainWindow(parent), ui(new Ui::MainWindow)
{
    ...
    object_timer->start(10);    //每10ms更新一次
...
void MainWindow::game_stop()
{
    ...
    {
        // die 撞到障礙物，失敗
        ui->label_title->setPixmap(QPixmap(":/src/game_falied.png"));   //設定標題圖面
    }
...
{
    int car_new_pos;
    car_new_pos = car_pos + car_direction;  //將接下來要移動到的位置暫存起來
    if((car_new_pos) >= 30 && (car_new_pos) <= 410)
    {                                           //判斷接下來要移動的位置是否超過跑道上下邊緣
        //未超過邊緣
...
void MainWindow::game_start()
{
    ...
    clock_timer->start(1000);                   //每隔1000ms更新一次
